#!/usr/bin/env python3
"""
Numerical Experiment - Liu et al. Paper Reproduction.

Reproduces Table 1 from the IEEE paper (DOI: 10.1109/TGRS.2025.3601233)
on fluorescence escape ratio.

Based on numerical_experiment_mod.py but with:
- Parameter grid matching Liu et al. Table 1
- Soil spectrum loaded from soilnew.txt (spectrum 2)
- Relative azimuth angle (RAA/psi) as a varied parameter
"""
...
Cab_values = [20, 40, 60, 80]
LAI_values = [0.25, 0.5, 0.75, 1, 1.5, 2, 3, 4, 5, 6, 7]
LAD_types = {
    'planophile':   (1.0, 0.0),
    'erectophile':  (-1.0, 0.0),
    'plagiophile':  (0.0, -1.0),
...
tts_values = [20, 30, 40, 50, 60]
tto_values = [0, 15, 30, 45, 60]
psi_values = [0, 90, 180]
...
def load_soil_spectrum_from_file(spectral, input_dir=None):
    ...
    soil_refl[spectral.IwlT] = 0.06  # thermal reflectance
    return soil_refl
...
_worker_atmo = None
_worker_soil_refl = None


def _init_worker(spectral, optipar, atmo, soil_refl):
    global _worker_spectral, _worker_optipar, _worker_atmo, _worker_soil_refl
    _worker_spectral = spectral
    _worker_optipar = optipar
    _worker_atmo = atmo
    _worker_soil_refl = soil_refl
...
def _run_scenario(params):
    ...
    Args:
        params: tuple of (Cab, LAI, LAD_name, LIDFa, LIDFb, tts, tto, psi, Rin, Ta, Vcmax)

    Returns:
        dict with scalar results and spectral arrays
    """
    Cab, LAI, LAD_name, LIDFa, LIDFb, tts, tto, psi, Rin, Ta, Vcmax = params
    spectral = _worker_spectral
    optipar = _worker_optipar
    atmo = _worker_atmo
    soil_refl = _worker_soil_refl

    base = {
        'Cab': Cab, 'LAI': LAI, 'LAD': LAD_name, 'tts': tts,
        'tto': tto, 'psi': psi, 'Rin': Rin, 'Ta': Ta, 'Vcmax': Vcmax,
    }
    nan_result = {
        **base,
    ...
    try:
        ...
        canopy = Canopy(
            LAI=LAI, hc=hc, LIDFa=LIDFa, LIDFb=LIDFb,
        ...
        soil_obj.Tsold = Ta * np.ones((12, 2))
        soil_obj.refl = soil_refl.copy()

        meteo = Meteo(z=10.0, Rin=Rin, Rli=300.0, Ta=Ta, p=970.0,
                      ea=15.0, u=2.0, Ca=410.0, Oa=209.0)

        angles = Angles(tts=tts, tto=tto, psi=psi)
        ...
        output = run_single_scenario(
            leafbio=leafbio, canopy=canopy, soil=soil_obj, meteo=meteo,
...
def run_experiment(output_dir=None, max_scenarios=None, atmfile=None, nworkers=None, input_dir=None):
    ...
    param_grid = []
    for combo in product(Cab_values, LAI_values, LAD_types.keys(), tts_values,
                         tto_values, psi_values, Rin_values, Ta_values, Vcmax_values):
        Cab, LAI, LAD_name, tts, tto, psi, Rin, Ta, Vcmax = combo
        LIDFa, LIDFb = LAD_types[LAD_name]
        param_grid.append((Cab, LAI, LAD_name, LIDFa, LIDFb, tts, tto, psi, Rin, Ta, Vcmax))
    ...
    pool = mp.Pool(nworkers, initializer=_init_worker,
                   initargs=(spectral, optipar, atmo, soil_refl))
    try:
    ...
    outfile = os.path.join(output_dir, f'numerical_experiment_liu_{timestamp}.csv')
    df.to_csv(outfile, index=False)
    print(f'Saved scalar results: {outfile}')

    # Save spectral data to NPZ file
    specfile = os.path.join(output_dir, f'numerical_experiment_liu_spectral_{timestamp}.npz')
    np.savez_compressed(
        specfile,
        wl=spectral_data['wl'],
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run SCOPE numerical experiment for Liu et al. paper')
